...
public class Achievement {
	...
	public static void init(Handler game, AchievementManager am) {
		...
		File f = new File(Game.prefix + "/00.arv");
		if (!f.exists()) {
			Utils.fileWriter("0", Game.prefix + "/00.arv");
		}
		f = new File(Game.prefix + "/01.arv");
		if (!f.exists()) {
			Utils.fileWriter("", Game.prefix + "/01.arv");
		}
		Achievement.am = am;
		...
		String s = FileLoader.readFile(Game.prefix + "/01.arv");
	...
	public static void award(Achievement a) {
		...
		if (!FileLoader.readFile(Game.prefix + "/01.arv").contains(a.name)) {
			int prev = Utils.parseInt(FileLoader.readFile(Game.prefix + "/00.arv", false));
			game.log("You earned: " + a.value);
			Utils.fileWriter("" + (prev + a.value), Game.prefix + "/00.arv");
			Utils.fileWriter(FileLoader.readFile(Game.prefix + "/01.arv") + System.lineSeparator() + a.name,
					Game.prefix + "/01.arv");
		}
