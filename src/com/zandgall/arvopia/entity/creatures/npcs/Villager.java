...
public class Villager extends NPC {
	...
	public Villager(Handler handler, double x, double y) {
		super(handler, x, y, 1.0D, "Villager ", new String[] { "Goodbye!" });
		this.type = Public.randInt(7);
		...
		texture = ImageLoader.loadImage("/textures/NPCs/Villagers/Variant" + type + ".png");
	...
	public void aiMove() {
		if ((right) && (r)) {
			...
			if (!collisionWithTile(tx, (int) (y + bounds.y - 18.0D) / Tile.HEIGHT)) {
				u = true;
			} else {
				r = false;
				l = true;
		...
		int ty = (int) ((y + yMove + bounds.y + bounds.height) / Tile.HEIGHT) + 6;
		if ((!collisionWithDown((int) ((x + bounds.x - 20.0D) / Tile.WIDTH), ty))
				&& (!collisionWithDown((int) ((x + bounds.x + bounds.width - 20.0D) / Tile.WIDTH), ty))
				&& (y + bounds.y + bounds.height < ty * Tile.HEIGHT + 4)) {
			r = true;
			l = false;
		} else if ((!collisionWithDown((int) ((x + bounds.x + 24.0D) / Tile.WIDTH), ty))
				&& (!collisionWithDown((int) ((x + bounds.x + bounds.width - 24.0D) / Tile.WIDTH), ty))
				&& (y + bounds.y + bounds.height < ty * Tile.HEIGHT + 4)) {
			r = false;
			l = true;
		}
