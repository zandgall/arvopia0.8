...
public class Storm {
	...
	long lTimer = 0L;
	long lTimed = (long) Public.expandedRand(100.0D, 1000.0D);
	...
	public Storm(Handler game, int ammount) {
		...
		for (int i = 0; i < ammount; i++) {
			clouds.add(new Cloud(game, (int) Public.expandedRand(-360.0D, 1080.0D), (int) Public.expandedRand(-400.0D, 0.0D),
					(int) Public.expandedRand(0.0D, 3.0D), Public.expandedRand(0.5D, 2.0D)));
		}
	}
	...
	public void tick(int speed) {
		...
		if ((lTimer >= lTimed) && (l == null)) {
			l = new Lightning(game, (int) Public.expandedRand(0.0D, World.getWidth() * Tile.WIDTH));
			lTimed = (long) (Public.expandedRand(100.0D, 1000.0D));
			lTimer = 0L;
		}
	...
	public void render(Graphics2D g2d) {
		...
		for (Cloud c : clouds)
			c.render(g2d);
		g2d.drawImage(ImageLoader.loadImage("/textures/StormOverlay.png"), 0, 0, null);
		if (l != null) {
			l.render(g2d);
		}
	}
	...
	public class Droplet {
		...
		public Droplet(Handler game, int x, int y, double size) {
			...
			c = new Color((int) Public.expandedRand(0.0D, 5.0D), (int) Public.expandedRand(0.0D, 20.0D),
					(int) Public.expandedRand(20.0D, 100.0D), (int) Public.expandedRand(50.0D, 200.0D));
		}
		...
		public void render(Graphics2D g) {
			...
			g.translate(x, y);
			g.setColor(c);
			g.fillRect(0, 0, (int) Math.max(1.0D, size / 4.0D), (int) size);
			g.setTransform(p);
		}
	}
	...
	public class Cloud {
		...
		public Cloud(Handler handler, double x, double y, int type, double speed) {
			...
			double d = Public.rand(1.0D, 2.0D);
			width = ((int) (54.0D * (d + Public.rand(0.0D, 0.5D))));
			height = ((int) (36.0D * (d + Public.rand(0.0D, 0.5D))));
		}

		public void tick() {
			x += speed + game.getWorld().getEnvironment().getWind(x, y);
			if (x > World.getWidth() * Tile.WIDTH + game.getWidth() / 2) {
				x = -54.0D;
			}
		}
		...
		public void render(Graphics2D g) {
			...
			g.drawImage(Tran.flip(cloud, widthflip, heightflip), 0, 0, width, height, null);
			g.setTransform(p);
		}
	}
	...
	public class Lightning {
		...
		public Lightning(Handler game, int x) {
			...
			touchPoint = (game.getWorld().getHighest(x) * Tile.HEIGHT);
		...
		private void create1() {
			int bendOff = (int) Public.expandedRand(x - 20, x + 20);
		...
		public void render(Graphics2D g2d) {
			g2d.setColor(new Color(200, 255, 255, (int) alpha));
